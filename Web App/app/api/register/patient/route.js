...
export async function POST(req) {
    try {
        ...
        if (existedPat || existedDoc) {
            return NextResponse.json({success:false, message: "User Already Existed" }, { status: 403 })
        }
        ...
        return NextResponse.json({success:true, message: "User registered.", data: newPatient }, { status: 201 });
    } catch (error) {
